...
import { PILLAR_UI } from "@/lib/pillar-ui";
...
export function GlossaryAccordion({ terms }: { terms: GlossaryTerm[] }) {
  return (
    <Accordion
      multiple={false}
      className={cn(
        "mt-6 grid w-full list-none grid-cols-1 gap-6 p-0 md:grid-cols-2 md:gap-x-8 md:gap-y-6",
      ...
      {terms.map((term) => (
        <AccordionItem
          ...
          className={cn(
            ...
            "flex flex-col rounded-xl border border-border/60 bg-card shadow-sm ring-1 ring-foreground/4",
          )}
        >
          <AccordionTrigger
            className={cn(
              "rounded-t-xl border-0 px-4 py-4 hover:bg-muted/30 hover:no-underline sm:px-5",
              "focus-visible:rounded-xl focus-visible:ring-2 focus-visible:ring-ring focus-visible:ring-offset-2 focus-visible:ring-offset-background cursor-pointer",
            )}
          >
            ...
            <div className="px-4 pb-4 pt-3 sm:px-5">
              <ul className="flex flex-wrap gap-1.5 pb-3" aria-label="Pillars">
                {term.categories.map((label) => {
                  const { icon: Icon, idle } = PILLAR_UI[label];
                  return (
                    <li key={label}>
                      <Badge
                        variant="outline"
                        size="xs"
                        className={cn("pointer-events-none gap-1 font-semibold", idle)}
                      >
