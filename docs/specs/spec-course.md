...
| **3. Deep dive: SDD**            | Spec is the primary artifact. Lifecycle: **specify → plan → task → implement**. Case: secure login or payments so agents cannot invent edge cases.                                            |
...
| **5. Strategic implementation**  | Decision framework: money/security → SDD; year of maintenance → SDD; prototypes may vibe; production systems measure twice.                                                                   |
