...
- **Navigation:** Header uses **three regions**: **logo** (left, links home), **primary nav** (centered on `md+`; **Glossary, Ecosystem, Videos, Course, Blog** — no separate Home item; insert **Tests** after Ecosystem when [spec-tests.md](./spec-tests.md) P1 ships), **Sign in** (right: **primary-colored border + text**, **taller control** `~h-10`, **Lucide `LogIn`** icon + label, light shadow). Below `md`, primary links move into a **sheet**; **Sign in** stays visible beside the menu trigger. No horizontal scroll for the header chrome at common mobile widths (~375px).
