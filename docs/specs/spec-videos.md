...
- **Search + topic filters** on `/videos`, same interaction as Glossary (query + Product / Design / Build / Quality chips, `?pillars=` in the URL). **Sort** (featured, newest, title) and **pagination** (12 / 24 / 48 per page, `?sort=` / `?page=` / `?per=`) ship on the static list. Catalog-in-Postgres remains in [spec-videos-v2-v3.md](./spec-videos-v2-v3.md).
...
TTL, favorites, badges, and Postgres catalog remain in **[spec-videos-v2-v3.md](./spec-videos-v2-v3.md)**. Search, topic filter, sort, pagination, suggest-a-video (with link preview), and auth-backed subscribe on the static list are **current**.