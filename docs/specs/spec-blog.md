...
| -------------------- | -------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `title`              | yes      | **SEO:** `<title>` / Open Graph. Human line, **50–58 characters**, **ends with a period**. Varied openers. **No** brand suffix. On-page headline is `heading` / `name`. |
| `slug`               | yes      | URL segment: `/blog/[slug]`                                                                                                                                                                                                                                                                                                                                             |
| `date`               | yes      | ISO date string                                                                                                                                                                                                                                                                                                                                                         |
| `description`        | yes      | **SEO:** meta description, **140–155 characters**, matching the title. Not required to double as on-page copy when `anons` is set. |
...
- Render full article.
- `**generateMetadata`:** `title` (human, **50–58 characters**, **ends with a period**) + `description` (**140–155 characters**); `**openGraph`** / `**twitter**` via `**shareMetadata**` and `**blogShareImagePath**`. See [spec-seo-meta.md](./spec-seo-meta.md). Keyword clusters from `**clustersForPostSlug**` plus optional frontmatter `keywords`.
- **JSON-LD:** `schema.org/Article` via `articleJsonLd` (`lib/json-ld.ts`) on the detail page, plus `schema.org/BreadcrumbList` (`breadcrumbJsonLd`: Home → Blog → article heading). Structured data **keeps Home** as the first item for crawlers even though the **visible** trail does not (see Header). Site-wide `WebSite` JSON-LD lives in the root layout.
...
- **Header:** Breadcrumbs **Blog →** article heading (current page, not a link; not a “Back to blog” control). **Do not** add a Home crumb — the site **logo** already links to `/` per [spec-design-layout.md](./spec-design-layout.md). Meta row (date + reading time); `<h1>` from `heading` (then `name`, then `title`); lead from `anons` (then `description`). **No** full-bleed hero image in the header—figures appear only inside the article markdown body.
